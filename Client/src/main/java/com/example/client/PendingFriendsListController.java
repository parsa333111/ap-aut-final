...
public class PendingFriendsListController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        for(String friend : connectedClient.getClient().getFriendRequests()) {
            ...
            try {
                friendInfo.getChildren().add(GetClientPhoto.GetImage(friend));
            } catch (IOException | ClassNotFoundException e) {
                throw new RuntimeException(e);
            }
            friendInfo.getChildren().add(textFriend);
            friendInfo.getChildren().add(new Text("  "));
            String path = ShowPictureHandler.getPath("cross.jpg");
            ...
            ImageView cross = new ImageView(localUrl);
            path = ShowPictureHandler.getPath("tick.jpg");
            file = new File(path);
            try {
                localUrl = file.toURI().toURL().toString();
            } catch (MalformedURLException e) {
            ...
            ImageView tick = new ImageView(localUrl);
            cross.setId(RandomToken.randomString(21));
            tick.setId(RandomToken.randomString(21));
            cross.setOnMouseClicked(mouseEvent -> {
                int index = 0;
                for(HBox hBox1 : listOfPendingFriend.getItems()) {
                    String choiceUserName = null;
                    for(Node node : hBox1.getChildren()) {
                        ...
                        if (node instanceof ImageView) {
                            ...
                            if(node.getId().equals(clicked.getId())) {
                                listOfPendingFriend.getItems().remove(index);
                                System.out.println("Successfully Add Friend");
                                System.out.println(choiceUserName);
                                try {
                                    connectedClient.getOutputStream().writeObject(new MsgRemoveRequest(connectedClient.getId(), connectedClient.getClient().getUsername(), friend));
                                } catch (IOException e) {
                                    throw new RuntimeException(e);
                                }
                                /*
            ...
            tick.setOnMouseClicked(mouseEvent -> {
                int index = 0;
                for(HBox hBox1 : listOfPendingFriend.getItems()) {
                    String choiceUserName = null;
                    for(Node node : hBox1.getChildren()) {
                        ...
                        if (node instanceof ImageView) {
                            ...
                            if(node.getId().equals(clicked.getId())) {
                                ...
                                System.out.println("Successfully Add Friend");
                                try {
                                    connectedClient.getOutputStream().writeObject(new MsgAddFriend(connectedClient.getId(), connectedClient.getClient().getUsername(), friend));
                                } catch (IOException e) {
                                    throw new RuntimeException(e);
                                }
                                /*
            ...
            friendInfo.getChildren().add(new Text("                                       "));
            friendInfo.getChildren().add(tick);
            friendInfo.getChildren().add(new Text("    "));
            friendInfo.getChildren().add(cross);
