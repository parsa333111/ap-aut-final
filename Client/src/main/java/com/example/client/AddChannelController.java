...
public class AddChannelController implements Initializable {
    ...
    void done(MouseEvent event) throws IOException {
        ConnectHandler.client.getOutputStream().writeObject(new MsgAddChannel(ConnectHandler.client.getId(), ConnectHandler.client.getClient().getUsername(), ConnectHandler.server.getId(), textField.getText()));
        try {
            client.getOutputStream().writeObject(new MsgGetServer(client.getId(), client.getClient().getUsername(), ConnectHandler.server.getId(), null));
        } catch (IOException e) {
            throw new RuntimeException(e);
