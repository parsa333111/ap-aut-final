...
public class ShowBlockListController implements Initializable {

    private Client connectedClient;
    @FXML
    private ListView<HBox> listOfBlocks;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        for(String friend : connectedClient.getClient().getBlocks()) {
            ...
            friendInfo.getChildren().add(new Text(friend));
            ...
            cross.setOnMouseClicked(mouseEvent -> {
                int index = 0;
                for(HBox hBox1 : listOfBlocks.getItems()) {
                    ...
                    for(Node node : hBox1.getChildren()) {
                        ...
                        if (node instanceof ImageView) {
                            ...
                            if(node.getId().equals(clicked.getId())) {
                                listOfBlocks.getItems().remove(index);
                                System.out.println("Successfully remove Block");
                                System.out.println(choiceUserName);
                                try {
                                    connectedClient.getOutputStream().writeObject(new MsgRemoveBlock(connectedClient.getId(), connectedClient.getClient().getUsername(), friend));
                                } catch (IOException e) {
                                    throw new RuntimeException(e);
                                }
                                return;
            ...
            listOfBlocks.getItems().add(friendInfo);
        }
