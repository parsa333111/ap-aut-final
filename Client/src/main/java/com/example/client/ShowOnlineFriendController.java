...
public class ShowOnlineFriendController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        try {
            ...
            for (String friend : connectedClient.getClient().getFriends()) {
                connectedClient.getOutputStream().writeObject(new MsgIsOnline(connectedClient.getId(), friend, ""));
                Msg msg = (Msg) connectedClient.getObjectInputStream().readObject();
                String status = "";
                if (msg.getText().equals("Online")) {
                    connectedClient.getOutputStream().writeObject(new MsgGetStatus(connectedClient.getId(), friend, ""));
                    msg = (Msg) connectedClient.getObjectInputStream().readObject();
                    status = msg.getText();
                } else {
                    status = "Offline";
                }
                System.out.println(friend + " " + status);
            ...
            for (String server : connectedClient.getClient().getServers()) {
                ...
                MsgGetServer msgGetServer = (MsgGetServer) connectedClient.getObjectInputStream().readObject();
