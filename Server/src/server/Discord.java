...
public class Discord {
     ...
     * @param msg to send
     */
    public static void sendToAllInChat(String sender, MsgChatMassage msg) {
        Iterator iteratorClients = clients.iterator();
        while (iteratorClients.hasNext()) {
            ClientHandler clientHandler = (ClientHandler) iteratorClients.next();
            System.out.println(clientHandler.ip + " " + dataBase.getClient(clientHandler.getClientName()).getCurrentChat().equals(msg.getChatId()));
            if (dataBase.getClient(clientHandler.getClientName()).getCurrentChat().equals(msg.getChatId())) {
                clientHandler.sendToClient(msg);
            }
        }
    }
        ...
        public void run() {
            try {
                while (true) {
                    ...
                    String check = msg.run();
                    if(msg instanceof MsgSingInUser && check.equals("True")) {
...
//                        } else if(dataBase.checkRequest(dataBase.getClient(msg.getText()), msg.getOwner())) {
//                            msgSendToClient(msg.getId(), new Msg(msg.getId(), msg.getOwner(), "", "AddedRequest"));
